...
class GATModule(nn.Module):
    ...
    def forward(self, graph, x):
        ...
        alpha_u = self.attn_linear_u(x)
        alpha_v = self.attn_linear_v(x)
        alpha = ops.u_add_v(graph, alpha_u, alpha_v)
        alpha = self.attn_act(alpha)
        alpha = edge_softmax(graph, alpha)

        x = x.reshape(-1, self.head_dim, self.num_heads)
        x = ops.u_mul_e_sum(graph, x, alpha)
...
class TransformerAttentionModule(nn.Module):
    ...
    def forward(self, graph, x):
        ...
        alpha = ops.u_dot_v(graph, queries, keys) / self.head_dim ** 0.5
        alpha = edge_softmax(graph, alpha)

        x = ops.u_mul_e_sum(graph, values, alpha)
        x = x.reshape(-1, self.dim)
